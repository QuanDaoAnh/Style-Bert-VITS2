...
from style_bert_vits2.triton.client import call_token_triton, call_bert_triton
...
def extract_bert_feature(
    ...
    if device == "cuda" and not torch.cuda.is_available():
        device = "cpu"
    model = bert_models.load_model(language, device_map=device)
    bert_models.transfer_model(language, device)
...
def extract_bert_feature_triton(
    text: str,
    word2ph: list[int],
    language: Languages,
    assist_text: Optional[str] = None,
    ...
    style_res_mean = None
    input_ids, token_type_ids, attention_mask, _ = call_token_triton( text, return_type="pt", language=language)

    res = call_bert_triton( input_ids, token_type_ids, attention_mask, language)
    if assist_text:
        style_input_ids, style_token_type_ids, style_attention_mask, _ = call_token_triton(assist_text, return_type="pt", language=language)
        style_res = call_bert_triton(style_input_ids, style_token_type_ids, style_attention_mask, language)
        style_res_mean = style_res.mean(0)
    ...
    if assist_text:
        ...
        repeat_feature = res
    # phone_level_feature = torch.repeat_interleave(repeat_feature, word2phone, 0)
    phone_level_feature = np.repeat( repeat_feature, word2ph, 0)
    return phone_level_feature.T
...
def clean_text_triton(
    ...
    if language == Languages.JP:
        from style_bert_vits2.nlp.japanese.normalizer import normalize_text
        from style_bert_vits2.nlp.japanese.g2p import g2p_triton

        norm_text = normalize_text(text)
        phones, tones, word2ph, _ = g2p_triton(norm_text, use_jp_extra, raise_yomi_error)
    elif language == Languages.EN:
        from style_bert_vits2.nlp.english.normalizer import normalize_text
        from style_bert_vits2.nlp.english.g2p import g2p_triton

        norm_text = normalize_text(text)
        phones, tones, word2ph = g2p_triton(norm_text)
    elif language == Languages.ZH:
        from style_bert_vits2.nlp.chinese.g2p import g2p
        from style_bert_vits2.nlp.chinese.normalizer import normalize_text
...
def clean_text_with_given_phone_tone_triton(
    text: str,
    language: Languages,
    given_phone: Optional[list[str]] = None,
    given_tone: Optional[list[int]] = None,
    ...
    # 与えられたテキストをクリーニング
    norm_text, phone, tone, word2ph = clean_text_triton(
        text,
        language,
        use_jp_extra=use_jp_extra,
        raise_yomi_error=raise_yomi_error,
